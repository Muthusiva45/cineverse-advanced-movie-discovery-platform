...
function MovieDetails() {
  ...
  return (
    <motion.main animate={{ opacity: 1 }} exit={{ opacity: 0 }} initial={{ opacity: 0 }}>
      <section className="on-media relative overflow-hidden pt-28">
        {poster ? (
          <img alt="" className="absolute inset-0 h-full w-full scale-110 object-cover opacity-45 blur-xl" src={poster} />
        ) : null}
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_24%_28%,rgba(229,9,20,0.2),transparent_28rem)]" />
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl gap-8 px-4 pb-16 sm:px-6 md:grid-cols-[minmax(14rem,22rem)_1fr] lg:px-10">
          <motion.div animate={{ opacity: 1, y: 0 }} className="max-w-xs md:max-w-none" initial={{ opacity: 0, y: 24 }}>
            {poster ? (
              <img
                alt={movie.Title}
                className="aspect-[2/3] w-full rounded-lg border border-white/10 object-cover shadow-cinematic"
                src={poster}
              />
            ) : (
          ...
          <motion.div animate={{ opacity: 1, y: 0 }} className="self-end" initial={{ opacity: 0, y: 24 }} transition={{ delay: 0.08 }}>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a className="magnetic-button inline-flex h-12 items-center gap-2 rounded-md bg-cinema-red px-6 font-extrabold text-white" href={trailerUrl} rel="noreferrer" target="_blank">
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-10">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {facts.map(([label, value]) => (
            <div className="glass-panel rounded-md p-4" key={label}>
